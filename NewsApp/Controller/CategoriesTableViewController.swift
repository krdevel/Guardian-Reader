...
class CategoriesTableViewController: UITableViewController, APIServiceDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "categoryCell", for: indexPath) as! NameTableViewCell
        
        cell.nameLabel?.text = self.categories?[indexPath.row].webTitle
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(self.categories?.count ?? 0 > 0) {
            ...
            destination.title = category?.webTitle
            destination.id = category!.id
            destination.useSearchQuery = true
        }
    }
